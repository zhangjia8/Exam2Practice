...
class Box(object):
    ...
    def shrink(self, new_volume):
        ...
        if new_volume < len(self.contents):
            for k in range(new_volume, len(self.contents)):
                stu_to_return += self.contents[k]

            for k in range(new_volume):
                contents += self.contents[k]
            self.contents = contents
        self.volume = new_volume
        return stu_to_return
    ...
    def double_then_shrink(self, new_volume):
        ...
        #    TIME ESTIMATE:   5 minutes.
        # --------------------------------------------------------------
        n = len(self.double()) + len(self.shrink(new_volume))
        return n
    ...
    def steal(self, other_box):
        ...
        #    ** TWO **   LINES OF CODE.
        ################################################################
        content = self.append_string(other_box.contents)
        other_box.contents = content
